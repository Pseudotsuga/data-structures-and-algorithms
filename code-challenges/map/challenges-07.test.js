...
const charCode = (arr) => arr.map( character => character.charCodeAt());
...
const extractAbilities = (arr) => arr.map( slot => slot = slot.ability.name);
...
const extractStats = (arr) => arr.map( statObj => statObj = {name: statObj.stat.name, total: statObj.effort + statObj.baseStat,});
