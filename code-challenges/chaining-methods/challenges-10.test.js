...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map( intArray => {
    let minusfive = intArray.filter( number => number % 5 === 0 && typeof number === 'number');
    return minusfive.map( newArrNumbers => Math.pow(2, newArrNumbers));
  });
};
...
let findMaleAndFemale = (data) => {
  return data.reduce((acc, val, index) => {
    if(val.gender === 'female' || val.gender === 'male') acc += val.name;
    if (index < data.length -1 && (val.gender === 'female' || val.gender === 'male')) acc += ' and ';
    else acc += '';
    return acc;
  }, '');
};
