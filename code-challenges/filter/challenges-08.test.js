...
const oddValues = (arr) => arr.filter( number => number % 2 === 0 ? false : true);
...
const getBaseStatGreaterThan = (arr, minBaseStat) => arr.filter( stat => stat.baseStat > minBaseStat ? true : false);
...
const getCharactersWithoutChildren = (arr) => arr.filter( character => character.children ? false : true);
