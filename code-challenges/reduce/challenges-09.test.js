...
const countNumberOfElements = (arr) => arr.reduce( (counter, value) => {
  value = 1;
...
const countNumberOfChildren = (arr) => arr.reduce( (accumulator, value) => value.children ? accumulator + value.children.length : accumulator, 0);
...
const countPrimeNumbers = (arr) => arr.reduce( (acc, val) => (isPrime(val)) ? acc += 1 : acc, 0);
...
const extractStat = (statName, arr) => arr.reduce((acc, val) => {
  return val.stat.name === statName ? acc = val:
    acc;
}, null);
